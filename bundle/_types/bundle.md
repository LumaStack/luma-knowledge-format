...
fields:
  ...
  applies_to: { obligation: optional,    field_type: list of text, desc: "Kinds of consumer that may adopt this Bundle. Open vocabulary. §11.1." }
---
...
`applies_to` is an **open vocabulary and LKF defines no values for it.** Where a
...
It is a list because a Bundle may legitimately apply at more than one, which is
the whole reason it is a field rather than a directory a distributor sorts into.
A directory can express only one, forcing the *publisher* to answer a question
that often belongs to the *adopter*. Absence says nothing — not "none" and not
"all" — and no consumer rejects a Bundle for it (§4).
