...
- **A Type Definition is a folder** *(breaking)*. `type_definitions/<name>/DEFINITION.md` replaces `type_definitions/<name>.md` — a type is a contract, and a contract carries more than its current text: a `CHANGELOG.md` (SHOULD be present — a contract's consumers are exactly the readers a change surprises, and the folder makes that history travel with the type wherever it is vendored), a `migrations/` directory for Documents describing how to bring a Document written against an earlier version up to the current one, and prior versions kept beside the definition as `DEFINITION-<version>.md` — the definition at that version, addressed by path like every reserved name. `DEFINITION.md` is the folder's one required file and the only name resolution reads; it joins the reserved files, speaking for the folder that holds it exactly as `BUNDLE.md` speaks for its Bundle, and `CHANGELOG.md` joins them with its casing arriving from the outside convention it shares. The built-in types carry per-type changelogs like any other — they are not special — keyed on each type's own `version`. What *runs* a migration stays a consumer's business — the format says only where one may live — and the shapes of `CHANGELOG.md` and `migrations/` contents are not yet fully specified, as with `LOG.md` and `INDEX.md`.
  *Migration:* for each Type Definition, make a folder named for the type and move the file into it as `DEFINITION.md` — `type_definitions/lab_result.md` becomes `type_definitions/lab_result/DEFINITION.md` — and start its `CHANGELOG.md`. A consumer that has not updated finds no file at the old path and resolves no bundle-local types — the safe direction, as above.

### Added
- **Every built-in Type Definition declares its own `version`, independent of `lkf_version`**, starting at `0.0.1`. The built-ins are the copied-type case the `version` field was specified for — vendored into bundles everywhere — so a format release that never touched their contracts must not report every copy as out of date. `0.0.1` is the earliest, most-unstable semver tier — the same place the format itself started — and deliberately not the format's own current number, because independence is the point. Their per-type changelogs are keyed on it, and name the format release that shipped each bump.
  *Migration:* none required. A vendored copy without `version` reads as before; re-vendoring picks the field up.

- **`type_version` joins the core fields, `recommended` everywhere.** A Document records which `version` of its type's Type Definition it was written against — copied from the number the definition declares, never minted — so migration becomes a query rather than a belief: when a contract moves, the Documents still on the old version are findable, and a tolerance can be retired when the query comes back empty instead of when everyone believes it would. Absence means unstated — a consumer MUST NOT infer a version from silence, and conformance is untouched. It records the single version the Document was last written or migrated against; whoever writes the Document writes it, and a tool that rewrites a file SHOULD keep it true. On a Type Definition it tracks the `type_definition` contract — the one kind of Document with two versions, its own `version` beside `type_version`, kept apart by one rule the format already follows in `BUNDLE.md`: bare `version` is what a Document publishes, a qualified `*_version` is what it conforms to. The built-ins, the specification and `BUNDLE.md` all carry it from this release.
  **Considered and not taken:** `type_definition_version` and `contract_version` (a Type Definition publishes a contract too, so both are ambiguous on the one file with two versions); `schema_version` (*schema* means validate-or-reject, which this format refuses); `defines_version` for the publishing side (self-labelling on the one rare two-version file, at the price of a second publishing spelling beside `BUNDLE.md`'s bare `version`, and against every package manifest in the wild); nesting the version under `type` or an in-band `type: lab_result@0.0.1` (the dispatch field is the one value every consumer compares, and changing its shape is the one migration field-tolerance cannot absorb — though in-band `@` is recorded in `roadmap.md` as not ruled out for a future major).
  *Migration:* none required — absence stays legal. Add `type_version` when a Document is next touched, copying its type's current `version`.
...
- **`survival` gains `temporary`, and `experimental` becomes `probationary`** *(breaking)*. The values are now `temporary | probationary | intended | promised` — four stances of one publisher toward one event, the thing ending, each defined by what it owes: an ending, a verdict, nothing, a process.
  **`experimental` failed the field's own grammar** — `intended` and `promised` are claims about the survival, but `experimental` describes the Document, and a Document can be experimental in nature with its survival `promised` (a permanent experiment). **The rename is a definition change, not a synonym**: the old rung opened *"No intentions"*, a shrug under which a Document three years in it was conforming; `probationary` owes a verdict, under which that Document is overdue — the trial is a deliberate arrangement whose decision comes from its results, and *is the period over?* becomes a checkable question, with `stale_after` as the existing way to schedule the review. Only the publisher renders the verdict; use still creates no undertaking.
  **`temporary` closes a recorded gap** — *never meant to last* was inexpressible: `archived` means retired, `stale_after` means recheck, and the old bottom rung meant only *might not survive*. It also makes a guardrail mechanical: a durable Document linking to a `temporary` one is a checkable defect. The line between the bottom two is whether the ending has been decided.
  **Considered and not taken**: `undecided`, `undetermined`, `unresolved` (owe nothing, so a Document could sit in them forever — indefinite limbo quietly lies about survival); `uncommitted`, `unpromised`, `contingent` (also true of `intended`, whose gloss is *nothing is promised*); `tenuous`, `fragile`, `at-risk`, `uncertain` (condition words leak across rungs — an `intended` thing can be tenuous in fact); `unearned` (misreads as a judgment that the thing survived undeservedly); `disclaimed` (fails the no-dictionary test every kept value passes); `unlikely`, `doomed`, `expiring` (predictions of death, a stronger claim than the rung makes). `intended` stays the default; `probationary` as default was rejected because a trial is an arrangement somebody has to actually open, and moving a default reinterprets every silent Document with no visible failure.
  *Migration:* rename `survival: experimental` to `survival: probationary`. A consumer that has not updated finds an unrecognised value, which fails visibly. Documents that never declared `survival` are unaffected — the default does not move.
...
- **`lifecycle_status` is renamed to `lifecycle`** *(breaking)*. Same values, same meaning, shorter name.
  ...
  **`lifecycle_stage` was considered and not taken.** It is more literal — the specification's prose calls the value *a Document's lifecycle stage*, and `lifecycle: draft` reads as *the lifecycle is draft* when `draft` is a position in one. **The name is a key rather than a sentence**, and it is typed in every Document that declares it; the extra word buys precision nobody was missing.
  **No transition period, and none is offered.** The `0.0.z` tier is unstable and says so, and a consumer that has not updated will find the field absent rather than misread — `lifecycle` is unrecognised where `lifecycle_status` was expected, which fails visibly rather than quietly.
...
- **`lifecycle` and `survival` are stated to answer different events.** They look like they overlap and they do not: **`lifecycle` governs the shape changing** and `stable` owes a path across; **`survival` governs the thing ending** and `promised` owes a process while `intended` owes nothing.
...
- **`survival` — a core field for how much you should expect a Document to last**. `experimental | intended | promised`, optional, default `intended`.
  ...
  **Neither it nor `lifecycle` is an input to the other**, and each is useful alone — a Document may declare either without the other. They are orthogonal in the sense *`verified` and trust tiers* already gives the word for trust: a Document can be `stable` and not expected to last, or `provisional` with its survival `promised`. Both corners are populated, which is what makes these two axes rather than one ladder.
