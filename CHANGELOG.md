...
### Added
- **Every built-in Type Definition declares its own `version`, independent of `lkf_version`**, starting at `0.1.0`. The built-ins are the copied-type case the `version` field was specified for — vendored into bundles everywhere — so a format release that never touched their contracts must not report every copy as out of date. `0.1.0` rather than `1.0.0` because a type's version claims nothing the format has not earned, and rather than the format's own number because independence is the point. Their per-type changelogs are keyed on it, and name the format release that shipped each bump.
