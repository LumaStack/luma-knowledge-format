...
It adds no fields of its own — a procedure needs a name, a line on when it
...
**This says nothing about when it loads.** `matches` says what makes it surface, and a consumer derives the rest. A procedure
...
**A procedure is transformed; no other Document in a Bundle is.** Consumers
...
For most Documents `description` is a convenience. For a procedure it is what a
...
A procedure says what to do. It does not say where it should be installed, in
...
A procedure declares **when its subject arises** — *before a release*, *at the
...
procedure is **not to run it**, which is the absence of an action. Detecting
...
arrives*.** A procedure's body may say so, and nothing mechanical follows.
