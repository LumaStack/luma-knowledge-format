...
A Bundle's own Document, living at the Bundle root as `BUNDLE.md`. It
...
Bundle's *content* version, distinct from `lkf_version`, which is the
...
and not "all" — and no consumer rejects a Bundle for it.

`description` is not declared here: it is a core field already, and
inheritance is add-only, so a type may not restate an inherited field to
